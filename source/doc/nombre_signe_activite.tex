...
\begin{NSI}
{Activité}{Encoder les nombres signés}
\end{NSI}

Les nombres entiers sont positifs ou négatifs. En informatique, ce sont des nombres \textbf{signés}. Comment sont-ils encodés sur une machine ?

Un nombre positif est encodé en binaire selon les règles de conversion vues précédemment.

Un nombre négatif est encodé par la méthode du \textbf{complément à 2}:

\begin{itemize}

\item On convertit en binaire la valeur absolue du nombre négatif à encoder;

\item On applique le \textbf{complément à 1} : chaque bit $0$ est remplacé par le bit $1$ et chaque bit $1$ est remplacé par le bit $0$;

\item Pour finir, on ajoute $1$ au complément à 1.
...
On encode le nombre $-4$ sur 4 bits.

\begin{itemize}

\item La valeur absolue $4$ s'écrit en binaire $0100$ sur 4 bits.

\item Le complément à 1 de $0100$ est $1011$

\item Le complément à 2 est $1011 + 0001 = 1100$.
\end{itemize}

Le nombre $-4$ est encodé en binaire par $1100$.
